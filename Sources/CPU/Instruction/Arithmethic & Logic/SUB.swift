...
struct CP: Instruction {
    ...
    func execute(with cpu: inout CPU) throws -> UInt16 {
        switch target {
        case .bit8(let uInt8):
            try Utils.cp(cpu: &cpu, value: uInt8, target: \.a)
        case .bit16(let uInt16):
            try Utils.cp(cpu: &cpu, value: UInt8(uInt16 & 0xF), target: \.a)
        case .bit8Target(let bit8Target):
            try Utils.cp(cpu: &cpu, value: cpu.registers[keyPath: bit8Target.registerKeypath], target: \.a)
        case .bit16Target(let bit16Target):
            let value = cpu.registers[keyPath: bit16Target.registerKeypath]
            try Utils.cp(cpu: &cpu, value: UInt8(value & 0xF), target: \.a)
        }
        return cycles
    }
}
